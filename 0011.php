...
for ($x = 0; $x < $length; $x++) {
    for ($y = 0; $y < $length; $y++) {
        if (($generated = check($input, $y, $x)) > $maximum) {
            $maximum = $generated;
        }
    }
}

echo "Computed maximum to be [{$maximum}]\n\n";

function check($i, $y, $x) {
    $down = 0;
    $diag = 0;
    $line = 0;
    $upri = 0;
    ...
    if (isset($i[$y+3][$x])) {
        $down = ($i[$y][$x] * $i[$y+1][$x] * $i[$y+2][$x] * $i[$y+3][$x]);
    }

    if (isset($i[$y+3][$x+3])) {
        $diag = ($i[$y][$x] * $i[$y+1][$x+1] * $i[$y+2][$x+2] * $i[$y+3][$x+3]);
    }

    if (isset($i[$y][$x+3])) {
        $line = ($i[$y][$x] * $i[$y][$x+1] * $i[$y][$x+2] * $i[$y][$x+3]);
    }

    if (isset($i[$y+3][$x-3])) {
        $upri = ($i[$y][$x] * $i[$y+1][$x-1] * $i[$y+2][$x-2] * $i[$y+3][$x-3]);
    }

    $max = max($down, $diag, $line, $upri);
    return $max;
}
